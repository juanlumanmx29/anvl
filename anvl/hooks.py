"""Hook management for Claude Code integration — inflation/churn alerts.

ANVL's single promise: warn the user when the conversation is *inflated*
(burning more tokens per turn than a fresh session would, or re-reading the
same files instead of producing edits). Context-window % is Claude's job —
it shows that natively. We don't duplicate it.

The hook never blocks. It prints a clear, prominent warning to stdout when
inflation or churn cross the threshold, and quietly auto-saves a handoff
on every turn so rotation is always one prompt away.
"""
...
def hook_entrypoint(can_block: bool = True) -> None:
    """UserPromptSubmit hook — pure warning, no blocking.

    Flow:
      1. `anvl bypass` in prompt → skip everything (escape hatch).
      2. Auto-save the handoff for this session (free, local).
      3. Compute inflation + churn.
      4. If either crosses threshold → print a clear warning to stdout.
      5. Context % is intentionally NOT a trigger — Claude shows it natively.
    """
    hook_input = {}
    try:
        raw = sys.stdin.read()
    ...
    jsonl_path, _session_id = result

    handoff_path = _auto_save_handoff(jsonl_path, cwd)

    from .parser import compute_churn_from_tools, worst_tier
    from .sessions import _quick_session_stats, compute_inflation_tier
    ...
    # Inflation + churn are the only triggers. Context % is informational
    # and shown by Claude natively — ANVL doesn't duplicate it.
    combined_tier = worst_tier(churn.health_tier, infl_tier)
    ...
    if churn.health_tier != "green":
        drivers.append(f"re-reads/edits ratio {churn.churn_score} — {churn.health_reason}")
    ...
    headline = {
        "yellow": "Sesión empezando a inflarse",
        "red": "Sesión inflada — conviene rotar",
        "critical": "Sesión muy inflada — rota ya",
    }.get(combined_tier, "Atención")

    bar = "═" * 60
    sep = "─" * 60
    lines = ["", bar, f"[ANVL] {icon} {headline}", ""]
    for d in drivers:
        lines.append(f"  • {d}")
    lines.append("")

    if rel_path:
        # Show the handoff path AND a copy-paste-ready prompt so the user
        # doesn't have to guess which handoff (out of many in this project)
        # to load in the new conversation.
        lines.append("  Handoff de ESTA sesión:")
        lines.append(f"    {rel_path}")
        lines.append("")
        lines.append("  Cuando abras conversación nueva, pega esto:")
        lines.append(f"  {sep}")
        lines.append(f"  Continúa desde el handoff: {rel_path}")
        lines.append(f"  {sep}")
    else:
        lines.append("  Handoff: no se pudo guardar — revisa .anvl/handoffs/ en este proyecto.")
    lines.append("")
    lines.append('  Para silenciar este turno: prefija tu mensaje con "anvl bypass".')
    lines.append(bar)
    lines.append("")
